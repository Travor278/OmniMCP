...
- **自研层**：`omni_mcp.py` 以单文件形式实现 54 个工具（15 个模块），覆盖从办公文档、图像处理、视频转码到 3D 建模、科学计算的完整能力栈。
...
| 条目 | 说明 |
|------|------|
| **核心服务** | `omni_mcp.py` — 54 个 MCP 工具，单文件实现，stdio 模式运行 |
...
| DOCX | 3 | 新建（段落/表格/图片/列表，内置学术/商务预设样式）、读取、查找替换 |
...
| SVG | 1 | 基于文本描述生成矢量图 |
| CHART | 2 | 单图绘制 + 多子图仪表板（支持 bar/line/scatter/pie/area/hist/heatmap/radar/box/stem/step） |
...
打开 `omni_mcp.py`，顶部 `CONFIG` 区域声明了各外部可执行文件的路径常量。服务器启动时会通过 `_find()` 函数在常见安装位置自动搜索，但建议手动核实：
...
- **作用**：本项目的核心，提供全部 54 个自研工具（详见上方能力矩阵）。
...
`omni_mcp.py` 约 1750 行，将 54 个工具、配置常量、辅助函数集中于同一文件。这在原型阶段便于快速迭代，但随着工具数量增长，代码导航、多人协作和单元测试隔离的成本会持续上升。
...
生成的 PPTX、DOCX、PDF 文件使用系统默认字体（Calibri、Arial、宋体）和简单的排版布局，没有 Office 主题引擎——颜色、字体、元素定位以功能可用为主，不是出版级品质。**建议把 OmniMCP 当作快速原型工具使用**：先用它生成结构和内容，再到原生编辑器（PowerPoint、Word、Acrobat 等）中精修样式。如需自定义字体或品牌色，可直接修改 `omni_mcp.py` 中的字体/颜色常量。
...
├── .vscode/
│   └── mcp.json                  # MCP 服务注册中枢
├── omni_mcp.py                   # 自研 MCP 服务器（54 工具，单文件实现）
...
本项目基于 [MIT License](LICENSE) 开源。欢迎使用、修改和贡献。
